...
import { useApiRouteStore } from '@/stores/apiRoute'
    ...
    const inputData = ref(JSON.parse(localStorage.getItem('myWorkingTimes') || "[]"));

    user.updateWorkingTimes().then((response) => {
        startFeching.value = false
        inputData.value =(JSON.parse(localStorage.getItem('myWorkingTimes') || "[]"));
    }).catch(() => {
        startFeching.value = false
    })
